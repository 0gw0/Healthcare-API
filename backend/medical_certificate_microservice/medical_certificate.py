...
def generate_certificate_amqp(request):
    # (1) Get the data from the JSON request body
...
def receiveOrderLog(channel):
    try:
        ...
        print('medical_certificate: Consuming from queue:', a_queue_name)
        channel.start_consuming()  # an implicit loop waiting to receive messages;
             #it doesn't exit by default. Use Ctrl+C in the command window to terminate it.
    
    except pika.exceptions.AMQPError as e:
        print(f"medical_certificate: Failed to connect: {e}") # might encounter error if the exchange or the queue is not created

    except KeyboardInterrupt:
        print("medical_certificate: Program interrupted by user.") 
    except json.decoder.JSONDecodeError:
        print("medical_certificate: Received an empty or invalid JSON message")


def callback(channel, method, properties, body): # required signature for the callback; no return
    generate_certificate_amqp(body)
    print()
...
if __name__ == "__main__":
    ...
    # AMQP start
    print("medical_certificate: Getting Connection")
    connection = amqp_connection.create_connection() #get the connection to the broker

    # AMQP settings
    channel = amqp_setup.create_channel(connection)
    amqp_setup.create_queues(channel)

    print("medical_certificate: Connection established successfully")
