...
timeslot_URL = "http://localhost:5002/timeslot"
...
def cancel_appointment(appointment_id):
    ...
    try:
        ...
        print()
        print("-- (1) Calling timeslot microservice --")
        print(f"Updating timeslot with timeslot_id={str(appointment_id)} to isAccepted=0 ...")
        response = invoke_http(f"{timeslot_URL}/update/{str(appointment_id)}",
                    method="PUT", json={"data": {"isAccepted": 0}})
        print(f"Update completed!")
        print(f"Response: {response}")
        print()
        ...
        response = invoke_http(f"{appointment_URL}/delete/{str(appointment_id)}",
                    method="DELETE")
